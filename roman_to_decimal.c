...
void decimalToWords(int num, char *words) {
    ...
    char chunkWords[256]; // Buffer for each chunk
    ...
    words[0] = '\0'; // Clear the output buffer at the start

    while (num > 0) {
        chunk = num % 1000; // Extract the last three digits
        if (chunk > 0) {
            chunkWords[0] = '\0'; // Clear chunkWords for the current chunk
            ...
            // Combine current chunk with previous result
            char combined[256] = "";
            sprintf(combined, "%s %s", chunkWords, temp);
            strcpy(temp, combined); // Store result back in temp
        }
    ...
    // Copy result to words and trim spaces
    strcpy(words, temp);
    // Remove extra spaces
    char *trimmed = words + strspn(words, " ");
    strcpy(words, trimmed);
}
